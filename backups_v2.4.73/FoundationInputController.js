...
window.FoundationInputController = {
    ...
    trySelectElement: function(mx, my, state) {
        ...
        for (const mh of (state.manholes || [])) {
            ...
            if (Math.hypot(mx - p.cx, my - p.cy) < HIT_PX) { 
        ...
        // 近傍のグリッド（レール）を特定。state.gridXCoords は GridEngine で全階層統合済み。
        const nearGridX = (state.gridXCoords || []).find(gx => Math.abs(mx - window.toCanvasPixel(gx, 0).cx) < HIT_PX);
        const nearGridY = (state.gridYCoords || []).find(gy => Math.abs(my - window.toCanvasPixel(0, gy).cy) < HIT_PX);
        ...
        for (const b of beams) {
            ...
            for (const seg of segments) {
                ...
                if (!isH && !isV) {
                    const p1c = window.toCanvasPixel(seg.p1.x, seg.p1.y);
                    const p2c = window.toCanvasPixel(seg.p2.x, seg.p2.y);
                    ...
                    if (Math.hypot(mx - (p1c.cx + t * (p2c.cx - p1c.cx)), my - (p1c.cy + t * (p2c.cy - p1c.cy))) < HIT_PX) {
                        window.PropertyController.showFdPopup('beam', b, mx, my);
                        return true;
                    }
                }
        ...
        for (const ew of (state.exteriorWalls || [])) {
            ...
            for (let i = 0; i < edgeVts.length - 1; i++) {
                const p1c = window.toCanvasPixel(edgeVts[i].x, edgeVts[i].y);
                const p2c = window.toCanvasPixel(edgeVts[i+1].x, edgeVts[i+1].y);
                const l2 = (p2c.cx - p1c.cx) ** 2 + (p2c.cy - p1c.cy) ** 2;
                const t = l2 > 0 ? Math.max(0, Math.min(1, ((mx - p1c.cx) * (p2c.cx - p1c.cx) + (my - p1c.cy) * (p2c.cy - p1c.cy)) / l2)) : 0;
                if (Math.hypot(mx - (p1c.cx + t * (p2c.cx - p1c.cx)), my - (p1c.cy + t * (p2c.cy - p1c.cy))) < HIT_PX) {
