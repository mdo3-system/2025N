...
window.FoundationEngine = {
    ...
    calculateSlabAnalysis: function(slabs, avgBuildingPressure) {
        ...
        slabs.forEach(slab => {
            ...
            // --- [重要改善] 局所的な建物荷重（1階のみ／2階あり）の算定ロジック ---
            const poly = slab.vertices.map(v => ({ x: v.x, y: v.y }));
            
            // 1階エリアポリゴン、2階エリアポリゴンを取得
            const a1Polys = (s.areaLines || []).filter(al => al.floor === '1F' && al.areaType !== 'attic' && al.areaType !== 'balcony');
            const a2Polys = (s.areaLines || []).filter(al => al.floor === '2F' && al.areaType !== 'attic' && al.areaType !== 'balcony');
            
            // 荷重定数 (kN/m2) - 建物仕様に基づき詳細化
            const wR = ( (s.config?.weights?.roof || 500) + (s.config?.weights?.solar || 0) + (s.config?.weights?.ceilingIns || 100) ) / 1000;
            const wW = ( (s.config?.weights?.exteriorWall || 600) + (s.config?.weights?.wallIns || 70) ) / 1000; // 外壁荷重を面積ベースで加算
            const wF = 2.4; // 骨組標準 (1層あたり)
            
            // スラブ面積のうち、1階がある部分と2階がある部分を算出
            const intersectArea = (p, areaPolys) => {
                let totalIntArea = 0;
                areaPolys.forEach(ap => {
                    if (!ap.vertices || ap.vertices.length < 3) return;
                    // 多角形クリッピングによる重なり面積の算出
                    let clipped = [...p];
                    const apVts = ap.vertices;
                    for (let i = 0; i < apVts.length; i++) {
                        const p1 = apVts[i], p2 = apVts[(i + 1) % apVts.length];
                        const dx = p2.x - p1.x, dy = p2.y - p1.y, L = Math.hypot(dx, dy);
                        if (L < 1) continue;
                        // CCW多角形において外側を指す法線 (dy, -dx) を使用
                        const nx = dy / L, ny = -dx / L, d = nx * p1.x + ny * p1.y;
                        clipped = this._clipByLine(clipped, nx, ny, -d); 
                    }
                    totalIntArea += M.polygonArea(clipped) / 1000000;
                });
                return totalIntArea;
            };

            const sArea1F = intersectArea(poly, a1Polys);
            const sArea2F = intersectArea(poly, a2Polys);
            const sAreaRoof = Math.max(sArea1F, sArea2F); 
            
            // 局所軸力の合算
            // 建物軸力 ΣN = (1F床面積 * (wF + wW)) + (2F床面積 * wF) + (屋根面積 * wR)
            // ※仕様合計に基づき、1階の面積分に外壁・骨組を、2階の面積分に骨組を、屋根の面積分に屋根荷重を割り振る
            const totalAxial_kN = (sArea1F * (wF + wW)) + (sArea2F * wF) + (sAreaRoof * wR);
            const axialPressure = area > 0 ? (totalAxial_kN / area) : 0;

            // (2) 立上り重量 (kN) 集計
            // スラブの境界にあるすべての梁（内部立ち上がり・外周立ち上がり共に集計）を抽出
            let stemWeight_kN = 0;
            const d_m = s.config?.fdThicknessM !== undefined ? s.config.fdThicknessM : 0.15;
            beams.forEach(b => {
            ...
            const slabMatWeight = 0; // スラブ自重は均し荷重に含めない
            ...
            p.detailedLoads = { wR, wW, wF, sArea1F, sArea2F, sAreaRoof };
            
            // ... (rest of the analysis remains same)
            const Mx_center = (p.support === '片持ち') ? 0 : 0; // dummy line for anchor
            // --- モーメント計算 (既存ロジック維持) ---
    ...
    getBeamPillars: function(beam, state) {
        ...
        const pillarsWithX = pillarsOnBeam.map(p => {
            ...
            const x_m = (clampedT * L_beam) / 1000;
            
            // [最重要] 柱の実際座標ではなく、梁軸への投影座標を「グローバル座標」として採用する
            // これにより、柱の数ミリのブレによる梁の折れ曲がり（クランク）を完全に防止する
            const projectedX = p1.x + clampedT * dx;
            const projectedY = p1.y + clampedT * dy;
    ...
    calculateSeismicForces: function(beam, pillars, state) {
        ...
        const getBeamGridAxisName = () => {
            ...
            const axisCounts = {};
            pillars.forEach(p => {
                const nm = window.GridEngine.getPillarName({ x: p.globalX, y: p.globalY }, s);
                if (nm && !nm.includes('位置不明')) {
                    const matches = nm.match(/[A-Z]+\d+/gi);
                    if (matches) matches.forEach(m => axisCounts[m] = (axisCounts[m] || 0) + 1);
                }
            });
            ...
            for (const axis in axisCounts) {
                if (axisCounts[axis] > maxCount) {
                    maxCount = axisCounts[axis];
                    bestAxis = axis;
                }
            }
            if (bestAxis) return bestAxis;

            // 2. 柱がない場合、梁の中点から最も近い通り芯を検索 (Fallback)
            const mid = { x: (beam.p1.x + beam.p2.x) / 2, y: (beam.p1.y + beam.p2.y) / 2 };
            const TOL = 250;
            ...
            if (isXHorizontal) {
                ...
                gYC.forEach((gy, i) => {
                    ...
                    if (d < minDist) { minDist = d; bestAxis = gYN[i]; }
                });
            } else {
                ...
                gXC.forEach((gx, i) => {
                    ...
                    if (d < minDist) { minDist = d; bestAxis = gXN[i]; }
                });
            }
            return (minDist < TOL) ? bestAxis : (bp.name || '');
        };
        const gridAxisName = getBeamGridAxisName();
