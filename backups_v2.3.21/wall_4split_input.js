...
function initCanvasInput(canvas) {
    ...
    canvas.addEventListener('mousedown', (e) => {
        ...
        if (e.button === 0) {
            ...
            // 統合選択モード または 仕様変更モード
            if (mode === 'select' || mode === 'edit-wall') {
                handleSelectMode(e, state);
                return;
            }

            // 柱削除
            if (mode === 'del-pillar' && hoveredPillar) {
                handlePillarDeletion(state);
                return;
            }
...
function handleSelectMode(e, state) {
    ...
    if (hit) {
        ...
        // 仕様変更モードの場合、右パネルのツールも同期させる
        if (getMode() === 'edit-wall' && hit.type === 'wall') {
            const w = hit.item;
            if (window.PropertyController) window.PropertyController.syncCenterPanel(w);
        }
    } else {
...
function handlePillarDeletion(state) {
    if (!hoveredPillar) return;
    hoveredPillar.isDeleted = true;
    // 関連する壁も削除フラグを立てるか検討（現在は物理削除を避ける設計）
    window.AppController.refreshAll();
}
