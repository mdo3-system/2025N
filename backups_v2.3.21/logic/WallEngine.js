...
window.WallEngine = {
    ...
    calculateDesignWalls: function(state) {
        ...
        ['1F', '2F'].forEach(f => {
            ...
            const cq = window.MathUtils.getVal(`c-q${suffix}`);
            const cw = window.MathUtils.getVal(`c-w${suffix}`);
            let awx = window.MathUtils.getVal(`a-wx${suffix}`);
            let awy = window.MathUtils.getVal(`a-wy${suffix}`);
            
            // 1F includes 2F's wind pressure area
            if (f === '1F') {
                awx += window.MathUtils.getVal('a-wx2');
                awy += window.MathUtils.getVal('a-wy2');
            }
