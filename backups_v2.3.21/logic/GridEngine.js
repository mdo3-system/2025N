...
window.GridEngine = {
    ...
    getPillarName: function(p, state) {
        if (!p) return '位置不明';
        let xi = state.gridXCoords.indexOf(p.x); 
        let yi = state.gridYCoords.indexOf(p.y);
    ...
    get4DivisionBounds: function(floor, state) {
        ...
        const zxt = getVal(`z-x-t${suffix}`) || (H / 4);
        const zxb = getVal(`z-x-b${suffix}`) || (H / 4);
        const zyl = getVal(`z-y-l${suffix}`) || (W / 4);
        const zyr = getVal(`z-y-r${suffix}`) || (W / 4);
