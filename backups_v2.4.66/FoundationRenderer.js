    ...
    drawTributary: function(state, toCanvas) {
        ...
        (state.foundationSlabs || []).forEach(slab => {
            ...
            slab.tributaryPolygons.forEach((tribEntry, i) => {
                ...
                const isTarget = selectedBeamId === tribEntry.beamId;

                ctx.save();
                if (isTarget) {
                    ctx.fillStyle = 'rgba(46, 204, 113, 0.3)'; // Highlight green
                    ctx.beginPath();
                    poly.forEach((v, idx) => {
                        const p = toCanvas(v, null);
                ...
                ctx.setLineDash([6, 3]); 
                ctx.strokeStyle = isTarget ? '#27ae60' : '#bdc3c7'; 
                ...
                if (pC.cx != null) {
                    ...
                    const labelA = `A = ${areaA.toFixed(3)} ㎡`;
                    const labelB = widthB > 0 ? `B = ${widthB.toFixed(3)} m` : '';

                    const wA = ctx.measureText(labelA).width;
                    const wB = labelB ? ctx.measureText(labelB).width : 0;
                    const maxW = Math.max(wA, wB);
                    
                    const paddingX = 6;
                    const paddingY = 4;
                    const rectW = maxW + paddingX * 2;
                    const rectH = (labelB ? 24 : 12) + paddingY * 2;
                    
                    const rx = pC.cx - rectW / 2;
                    const ry = pC.cy - rectH / 2;
                    ...
                    ctx.fillStyle = isTarget ? '#1e8449' : '#2c3e50';
    ...
    drawPreviews: function(state, toCanvas) {
        ...
        if (fm === 'f_beam' && fdSel) {
            ...
            if (sp.cx != null) {
                ...
                /* 
                const dx = Math.abs(previewEp.x - fdSel.x);
                const dy = Math.abs(previewEp.y - fdSel.y);
                if (dx > dy) {
                    previewEp.y = fdSel.y;
                } else {
                    previewEp.x = fdSel.x;
