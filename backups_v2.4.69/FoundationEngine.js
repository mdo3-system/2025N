...
window.FoundationEngine = {
    ...
    calculateSlabAnalysis: function(slabs, avgBuildingPressure) {
        ...
        slabs.forEach(slab => {
            ...
            const intersectArea = (p, areaPolys) => {
                ...
                areaPolys.forEach(ap => {
                    ...
                    for (let i = 0; i < apVts.length; i++) {
                        ...
                        const dx = p2.x - p1.x, dy = p2.y - p1.y, L = Math.hypot(dx, dy);
                        if (L < 1) continue;
                        // [修正] CCW多角形において外側を指す法線 (dy, -dx) を使用
            ...
            const sAreaRoof = Math.max(sArea1F, sArea2F); // 屋根は通常1階か2階の大きい方に載る
            
            // 局所軸力の合算
            // 2階がある部分は (wF*2 + wR)、1階のみの部分は (wF*1 + wR) となるように按分
            const totalAxial_kN = (sArea1F * wF) + (sArea2F * wF) + (sAreaRoof * wR);
            const axialPressure = area > 0 ? (totalAxial_kN / area) : 0;

            // (2) 立上り重量 (kN) 集計 (既存ロジック維持)
            let stemWeight_kN = 0;
    ...
    calculateSeismicForces: function(beam, pillars, state) {
        ...
        // Extract the actual grid axis name for the beam (e.g., Y1, X1)
        const getBeamGridAxisName = () => {
            // [改善] 始点・終点だけでなく、経路上にあるすべての柱から最も多い通り名を抽出する
            const axisCounts = {};
            pillars.forEach(p => {
            ...
            // Fallback to end point logic if no pillars
