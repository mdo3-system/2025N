    ...
    refreshAll: function(shouldSaveHistory = true) {
        ...
        if (window.AreaEngine) {
            ['1F', '2F'].forEach(f => {
                ...
                // Auto-fill 4-division side end areas
                const div4 = window.AreaEngine.calculate4DivisionAreas(f, state);
                if (div4) {
                    if (div4.xt > 0) { const el = document.getElementById('e-x-t' + lv); if (el) el.value = div4.xt.toFixed(3); }
                    if (div4.xb > 0) { const el = document.getElementById('e-x-b' + lv); if (el) el.value = div4.xb.toFixed(3); }
                    if (div4.yl > 0) { const el = document.getElementById('e-y-l' + lv); if (el) el.value = div4.yl.toFixed(3); }
                    if (div4.yr > 0) { const el = document.getElementById('e-y-r' + lv); if (el) el.value = div4.yr.toFixed(3); }
                }
            });
