    ...
    const collectAlpha = (targetPillarId, targetFloor, refP) => {
        ...
        walls.filter(w => w.floor === targetFloor && (w.p1.id === targetPillarId || w.p2.id === targetPillarId))
            .forEach(w => {
                ...
                let isBrace = w.braceId ? (w.braceId !== 'b0') : [1.0, 1.5, 2.0, 3.0, 4.0].includes(w.braceVal || 0);
                let braceObj = isBrace ? { m: (w.braceId ? window.getBraceSpec(w.braceId).val : (w.braceVal || 0)), type: isP1 ? 'P1' : 'P2' } : null;
