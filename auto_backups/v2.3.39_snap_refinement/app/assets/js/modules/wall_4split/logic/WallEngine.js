        ...
        const getAreaDef = (f, type, name) => {
            ...
            if (target.length > 0) {
                ...
                if (Math.abs(total - manualVal) < 0.01) {
                    let stepA = parts.length > 1 ? `[作図の${name}面積合算] = ${parts.map(p=>p.toFixed(2)+'㎡').join(' ＋ ')} = ${total.toFixed(2)}㎡` : `[作図の${name}面積] = ${total.toFixed(2)}㎡`;
                    return { val: manualVal, stepA };
                } else {
                    return { val: manualVal, stepA: `[手動修正済み] ${manualVal.toFixed(2)}㎡ (作図:${total.toFixed(2)}㎡)` };
                }
            }
