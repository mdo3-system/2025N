...
window.GridEngine = {
    ...
    analyzeGrids: function(state) {
        ...
        const TOL_SNAP = 10; // 近接グリッドのマージ制限 (1819->1820 修正のため10に拡大)
        const TEXT_GRID_TOL_WIDE = 300; // 文字検索の許容誤差

        // 座標を標準モジュール(455mm)に丸める補助関数
        const snapToModule = (val) => {
            const module = 455;
            const remainder = val % module;
            if (Math.abs(remainder) < TOL_SNAP) return val - remainder;
            if (Math.abs(remainder - module) < TOL_SNAP) return val + (module - remainder);
            if (Math.abs(remainder + module) < TOL_SNAP) return val - (module + remainder);
            return Math.round(val / module) * module; // より強制的にモジュールに合わせる
        };
