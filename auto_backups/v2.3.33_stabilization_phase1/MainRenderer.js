...
window.MainRenderer = {
    ...
    drawWallSymbols: function(ctx, w, p1, p2, state) {
        ...
        if (braceSpec.val > 0) {
            ...
            if (braceSpec.text.includes('たすき')) {
            ...
            state.pillars.filter(p => !p.isDeleted && !p.isInvalidPos && p.floor === state.currentFloor).forEach(p => {
                if (p.tributaryPolygon && p.tributaryPolygon.length > 0) {
                    ...
                    if (areaVal > 0) {
                        ...
                        ctx.strokeStyle = state.isPrintMode ? "#fff" : "#1e1e1e"; ctx.lineWidth = 3;
                        ctx.strokeText(txt, pt.cx, pt.cy + 10);
                        ctx.fillStyle = "#27ae60";
                        ctx.fillText(txt, pt.cx, pt.cy + 10);
                    }
                }
            });
            ctx.setLineDash([]);
