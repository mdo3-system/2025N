            ...
            ents.forEach(ent => {
                ...
                if (ent.type === 'INSERT') {
                    ...
                    if (L.includes('COL')) {
                        ...
                        if (ent.type === 'POINT') {
                            const px = ent.position ? ent.position.x : ent.x;
                            const py = ent.position ? ent.position.y : ent.y;
                            pillars.push({ id: `P${pIdCounter++}`, x: px, y: py, floor: f, layer: L });
                        } else if (ent.type === 'CIRCLE' && ent.radius < 500) {
                            pillars.push({ id: `P${pIdCounter++}`, x: ent.center.x, y: ent.center.y, floor: f, layer: L });
                            newBgLines.push(ent);
