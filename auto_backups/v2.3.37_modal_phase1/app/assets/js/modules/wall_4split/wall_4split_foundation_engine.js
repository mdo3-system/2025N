...
function runFoundationBeamAnalysis(beams, slabs) {
...
function getFoundationBeamReportHtml(beam) {
    ...
    const fmtR = (r) => {
        if (!isFinite(r)) return '<span style="color:#e74c3c;font-weight:bold;">—</span>';
        const pct = (r * 100).toFixed(1);
        const ok  = r <= 1.0;
        return `<span style="color:${ok ? '#27ae60' : '#e74c3c'};font-weight:bold;">${pct}% ${ok ? 'OK' : 'NG'}</span>`;
    };

    const span = s.spanM != null ? fmt(s.spanM, 2) : '—';
    const wL   = s.wL_kN_m != null ? fmt(s.wL_kN_m, 2) : '—';
    const tw   = (s.stressData && s.stressData.tributaryWidth_m) ? fmt(s.stressData.tributaryWidth_m, 2) : '—';

    const row = (label, v1, v2) => `<tr><td style="padding:4px 8px;background:#f8f9fa;font-weight:bold;white-space:nowrap;">${label}</td><td style="padding:4px 8px;text-align:right;">${v1}</td><td style="padding:4px 8px;text-align:right;">${v2}</td></tr>`;
    const sep = (label) => `<tr><td colspan="3" style="padding:3px 8px;background:#2c3e50;color:#fff;font-size:11px;font-weight:bold;">${label}</td></tr>`;

    return `
    <div style="font-family:'Hiragino Kaku Gothic ProN','Meiryo',sans-serif;font-size:12px;line-height:1.5;padding:4px;">
        <div style="background:#2c3e50;color:#fff;padding:8px 12px;border-radius:4px 4px 0 0;font-weight:bold;font-size:13px;">
            📐 基礎梁 断面検定結果
        </div>
        <div style="background:#ecf0f1;padding:6px 12px;font-size:11px;border-bottom:1px solid #bdc3c7;">
            ID: ${beam.id} &nbsp;|&nbsp; ${p.width || 150} × ${p.height || 640} (mm) &nbsp;|&nbsp; スパン: ${span} m
        </div>
        <table style="width:100%;border-collapse:collapse;border:1px solid #bdc3c7;">
            <thead>
                <tr style="background:#34495e;color:#fff;">
                    ...
                    <th style="padding:5px 8px;text-align:right;">長期</th>
                    <th style="padding:5px 8px;text-align:right;">短期</th>
                </tr>
            </thead>
            <tbody>
                ${sep('📦 断面情報')}
                ${row('梁幅 b (mm)', c.b, c.b)}
                ${row('梁成 h (mm)', c.h, c.h)}
                ${row('有効せい d (mm)', fmt(c.d, 0), fmt(c.d, 0))}
                ${row('下端主筋 At (mm²)', fmt(c.At, 1), fmt(c.At, 1))}
                ${row('せん断補強筋比 pw', fmt(c.pw, 4), fmt(c.pw, 4))}
                ${sep('⚖️ 荷重・応力')}
                ${row('負担幅 (m)', tw, tw)}
                ${row('線荷重 w (kN/m)', wL, '—')}
                ${row('最大曲げ M (kN·m)', fmt(Math.max(s.stressData.M_long_mid_Nmm, s.stressData.M_long_end_Nmm) / 1e6, 2), fmt((Math.max(s.stressData.M_long_mid_Nmm, s.stressData.M_long_end_Nmm) + s.stressData.M_short_end_Nmm) / 1e6, 2))}
                ${row('最大せん断 Q (kN)', fmt(s.stressData.Q_long_N / 1e3, 2), fmt((s.stressData.Q_long_N + s.stressData.Qe_N) / 1e3, 2))}
                ${sep('✅ 許容耐力・検定')}
                ${row('許容曲げ Ma (kN·m)', fmt(c.Ma_L / 1e6, 2), fmt(c.Ma_S / 1e6, 2))}
                ${row('許容せん断 Qa (kN)', fmt(c.Qa_L / 1e3, 2), fmt(c.Qa_S / 1e3, 2))}
                ${row('M 検定比 (M/Ma)', fmtR(s.ratioM_L), fmtR(s.ratioM_S))}
                ${row('Q 検定比 (Q/Qa)', fmtR(s.ratioQ_L), fmtR(s.ratioQ_S))}
            </tbody>
        </table>
        <div style="padding:6px 12px;font-size:11px;color:${s.isNG ? '#e74c3c' : '#27ae60'};font-weight:bold;background:${s.isNG ? '#fdf3f3' : '#f0fdf4'};border:1px solid ${s.isNG ? '#e74c3c' : '#27ae60'};border-top:none;border-radius:0 0 4px 4px;">
            ${s.isNG ? '⚠ 断面検定NG — 断面を大きくするか配筋を増やしてください' : '✔ 断面検定OK — 全て許容値内に収まっています'}
        </div>
    </div>`;
}
 ...
 */
function calculateFoundationSlabAnalysis(slabs, avgBuildingPressure) {
 ...
 */
function getFoundationSlabReportHtml(slab) {
...
function generateBeamNMQSvg(beam) {
    ...
    beam.spans.forEach((s, idx) => {
        ...
        const mScale = (hSection * 0.4) / maxM;
        const qScale = (hSection * 0.4) / maxQ;
        const nScale = (hSection * 0.3) / maxN;
        ...
        // 2. M図 (包絡線)
        const mL_mid = fs.stressData.M_long_mid_Nmm / 1e6;
        const mS_mid = (fs.stressData.M_short_end_Nmm || 0) / 2;
        ...
        // 3. Q図 (階段状/直線)
        const qL = fs.stressData.Q_long_N / 1e3;
        const qS = fs.stressData.Qe_N / 1e3;
        const qTotal = qL + qS;
