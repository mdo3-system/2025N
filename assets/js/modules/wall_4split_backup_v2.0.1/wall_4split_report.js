...
function getVal(id) { return window.UIController ? window.UIController.getVal(id) : 0; }
function getStr(id) { return window.UIController ? window.UIController.getStr(id) : ''; }
function getMode() { return window.UIController ? window.UIController.getMode() : 'wall'; }
...
function updateReport() {
    ...
    ['2F', '1F'].forEach(f => {
        ...
        let calcModeStr = window.AppState.uiParams.calcMode === 'seinou' ? '性能表示(見上げ面積)' : '建築基準法(見下げ面積)';
...
window.generateEnlargedAxialDiagramSvg = function(axisName, loadDirection = 'left') {
    ...
    const floorDataMap = [];
    const h1 = (window.AppState.uiParams['n-h1'] || 2.7) * 1000;
    const h2 = (window.AppState.uiParams['n-h2'] || 2.7) * 1000;
