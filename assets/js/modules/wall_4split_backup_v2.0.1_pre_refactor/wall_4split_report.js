...
// ★ 筋交い仕様のマスターリストを取得する関数 (設定ファイル相当)
window.getMasterBraceList = function() {
    return [
        { id: "b0", val: 0, text: "なし" },
...
window.getBraceSpec = function(id) {
    ...
    const master = window.getMasterBraceList();
    // IDが直接倍率（数値）の場合のフォールバック
    if (!isNaN(id) && id !== "") return { id: "custom", val: parseFloat(id), text: "カスタム" };
...
function getVal(id) { 
    ...
    if (!el) return 0;
    const val = el.value;
    // ID (b0, b1...) の場合はマスターから引く
    if (val && val.startsWith('b')) {
        return window.getBraceSpec(val).val;
    }
...
window.getWallTotalVal = function(w) {
    if (!w) return 0;
    // IDがある場合はマスターから最新値を引く
    if (w.outPanelId || w.inPanelId || w.braceId) {
        const p1 = window.getWallSpec(w.outPanelId);
        const p2 = window.getWallSpec(w.inPanelId);
        const b = window.getBraceSpec(w.braceId);
        return (p1.val || 0) + (p2.val || 0) + (b.val || 0);
    }
    // IDがない旧データの場合はキャッシュ値を返す
    return w.totalVal || 0;
};

// ★ 面材仕様のマスターリストを取得する関数 (設定ファイル相当)
window.getMasterWallList = function() {
    let customWalls = (typeof getCustomWallList === 'function') ? getCustomWallList() : [];
    let baseOptions = [
    ...
    customWalls.forEach((cw, idx) => {
        ...
        baseOptions.push({ id: cid, val: cw.val, text: `${mark}${cw.name} (${cw.val}倍)` });
    });
    return baseOptions;
};

// IDから仕様を取得する関数 (呼び出し形式の核)
window.getWallSpec = function(id) {
    if (!id) return { id: "opt0", val: 0, text: "なし" };
    const master = window.getMasterWallList();
    return master.find(m => m.id === id) || { id: "opt0", val: 0, text: "なし" };
};

// ★ 面材selectの倍率取得（value属性はID、実際の値はマスターから引く）
function getPanelVal(id) {
    let el = document.getElementById(id);
    if (!el || el.selectedIndex < 0) return 0;
    const spec = window.getWallSpec(el.value);
    return spec.val;
}
...
function updateWallSelects() {
    ...
    ['wall-p1', 'wall-p2'].forEach(id => {
        ...
        let html = master.map(opt => `<option value="${opt.id}" data-val="${opt.val}">${opt.text}</option>`).join("");
        sel.innerHTML = html;
