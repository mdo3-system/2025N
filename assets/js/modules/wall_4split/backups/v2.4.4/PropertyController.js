...
window.getFoundationSlabReportHtml = function(slab) {
    ...
    let html = `<div style="background:#f8f9fa; border:1px solid #dee2e6; border-radius:4px; padding:10px; font-size:11px; line-height:1.6;">`;
    html += `<div style="font-weight:bold; color:#2c3e50; border-bottom:1px solid #27ae60; margin-bottom:8px;">📋 断面検定計算書</div>`;
    
    // 1. 接地圧算定式
    html += `<div style="margin-bottom:10px;">`;
    html += `<div style="color:#27ae60; font-weight:bold;">1. 設計用接地圧 q の算定</div>`;
    html += `<div style="padding-left:10px;">`;
    html += `q = bp + w<sub>self</sub> + w<sub>live</sub><br>`;
    html += `= ${fmt(s.bp)} + ${fmt(s.wSelf)} + ${fmt(s.wLive)} = <b>${fmt(s.qTotal)}</b> kN/㎡`;
    html += `</div></div>`;

    // 2. モーメント算定式
    html += `<div style="margin-bottom:10px;">`;
    html += `<div style="color:#27ae60; font-weight:bold;">2. 曲げモーメント M の算定</div>`;
    html += `<div style="padding-left:10px;">`;
    if (s.cantileverLength) {
        html += `M = 0.5 × q × L²<br>`;
        html += `= 0.5 × ${fmt(s.qTotal)} × ${fmt(s.cantileverLength)}² = <b>${fmt(s.Mx_center)}</b> kNm`;
    } else {
        const alpha = Math.max(s.alpha_mcx||0, s.alpha_max||0);
        html += `M<sub>short</sub> = α × q × l<sub>x</sub>²<br>`;
        html += `= ${fmt(alpha, 3)} × ${fmt(s.qTotal)} × ${fmt(s.lx)}² = <b>${fmt(Math.max(s.Mx_center||0, s.Mx_end||0))}</b> kNm`;
    }
    html += `</div></div>`;

    // 3. 検定テーブル
    html += `<table style="width:100%; border-collapse:collapse; margin-bottom:8px; background:white; border:1px solid #ddd;">`;
    html += `<tr style="background:#eee;"><th style="text-align:left; padding:4px;">項目</th><th style="text-align:right; padding:4px;">設計 M</th><th style="text-align:right; padding:4px;">許容 Ma</th><th style="text-align:center; padding:4px;">判定</th></tr>`;
    html += `<tr><td style="padding:4px;">短辺 (kNm)</td><td style="text-align:right; padding:4px;">${fmt(Math.max(s.Mx_center||0, s.Mx_end||0))}</td><td style="text-align:right; padding:4px;">${fmt(s.Ma_short)}</td><td style="text-align:center; padding:4px;">${fmtR(s.ratioShort)}</td></tr>`;
    if (!s.cantileverLength) {
        html += `<tr><td style="padding:4px;">長辺 (kNm)</td><td style="text-align:right; padding:4px;">${fmt(Math.max(s.My_center||0, s.My_end||0))}</td><td style="text-align:right; padding:4px;">${fmt(s.Ma_long)}</td><td style="text-align:center; padding:4px;">${fmtR(s.ratioLong)}</td></tr>`;
    }
    html += `</table>`;
    
    if (s.isNG) html += `<div style="color:#e74c3c; font-weight:bold; text-align:center; border:1px solid #e74c3c; padding:4px; background:#fff5f5;">⚠️ 断面不足 (NG)</div>`;
    else html += `<div style="color:#27ae60; font-weight:bold; text-align:center; border:1px solid #27ae60; padding:4px; background:#f5fff5;">✅ 安全 (OK)</div>`;
