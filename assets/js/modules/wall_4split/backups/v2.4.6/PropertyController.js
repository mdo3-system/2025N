    ...
    generateBeamHtml: function(beam) {
        ...
        beam.spans.forEach((span, idx) => {
            ...
            if (st) {
                ...
                const Ne_kN = ((st.st?.Ne_N || 0) / 1000).toFixed(2);
                ...
                stressInfo = `<div style="line-height:1.4;">
                    ...
                    短期: <b style="color:#e67e22;">Ne=${Ne_kN} kN</b> (B=0.5)<br>
                    検定比: <b style="${parseFloat(ratio) > 1.0 ? 'color:red' : 'color:#2980b9'}">${ratio}</b>
                </div>`;

                // 負担面積の内訳 (Visibility Improvement)
                const state = window.AppState;
                const tribs = [];
                (state.foundationSlabs || []).forEach(slab => {
                    (slab.tributaryPolygons || []).forEach(tp => {
                        if (tp.beamId === beam.id) {
                            tribs.push({ name: slab.props?.name || 'S?', area: (tp.area/1e6).toFixed(2), width: (tp.width||0).toFixed(2) });
                        }
                    });
                });
                if (tribs.length > 0) {
                    tribInfo = `<div style="margin-top:5px; background:#f0fbf0; padding:5px; border-radius:3px; border:1px solid #c3e6cb; font-size:9px;">
                        <div style="font-weight:bold; color:#218838; margin-bottom:2px;">📐 負担荷重内訳:</div>
                        ${tribs.map(t => `${t.name}: ${t.area}㎡ (幅 ${t.width}m)`).join('<br>')}
                    </div>`;
                }
            }
            ...
            html += `<tr style="border-bottom:1px solid #eee;">
                ...
                <td style="padding:8px 5px; vertical-align:top; text-align:right;">
                    ${stressInfo}
                    ${tribInfo}
                </td>
        ...
        return `
            <div class="calc-box" style="padding:10px; max-height:600px; overflow-y:auto; font-family:sans-serif;">
                ...
                <div class="calc-row" style="margin-bottom:8px;">
                    <label style="width:80px; display:inline-block; font-size:11px;">スラブ名</label>
...
window.getFoundationSlabReportHtml = function(slab) {
    ...
    // (2) 荷重の内訳表
    html += `<table style="width:100%; border-collapse:collapse; margin-bottom:10px; border:1px solid #333; text-align:center;">`;
    html += `<tr style="background:#eee; border-bottom:1px solid #333;">`;
    html += `<th style="padding:4px; border-right:1px solid #333;">軸力 (kN)</th>`;
    html += `<th style="padding:4px; border-right:1px solid #333;">立上り (kN)</th>`;
    html += `<th style="padding:4px; border-right:1px solid #333;">均し荷重 (kN/㎡)</th>`;
    html += `<th style="padding:4px;">床荷重 (kN/㎡)</th>`;
    html += `</tr>`;
    html += `<tr>`;
    html += `<td style="padding:6px; border-right:1px solid #333;">${fmt(s.totalAxial_kN)}</td>`;
    html += `<td style="padding:6px; border-right:1px solid #333;">${fmt(s.stemWeight_kN)}</td>`;
    html += `<td style="padding:6px; border-right:1px solid #333; font-weight:bold;">${fmt(s.deadLoad, 3)}</td>`;
    html += `<td style="padding:6px;">${fmt(s.floorLoad, 3)}</td>`;
    html += `</tr>`;
    html += `</table>`;

    // (3) 接地圧の算定式表示
    html += `<div style="margin-bottom:12px; padding:8px; border-left:4px solid #27ae60; background:#f0fbf0;">`;
    html += `<div style="font-weight:bold; margin-bottom:4px;">接地圧 σe の算定</div>`;
    html += `均し荷重 = (軸力 + 立上り重量) / 面積 = (${fmt(s.totalAxial_kN)} + ${fmt(s.stemWeight_kN)}) / ${fmt(s.area)} = ${fmt(s.deadLoad, 3)}<br>`;
    html += `接地圧 σe = 均し荷重 + 床荷重 = ${fmt(s.deadLoad, 3)} + ${fmt(s.floorLoad, 3)} = <b>${fmt(s.qTotal, 3)}</b> kN/㎡`;
    html += `</div>`;
    ...
    // 接地圧判定 (fe'は仮に46.4固定、または設定値があればそれを使用)
    const fe_prime = 46.4; 
    const groundRatio = s.qTotal / fe_prime;
    html += `<tr style="border-bottom:1px solid #ddd;"><td style="padding:4px; border-right:1px solid #333;">接地圧 σe / fe'</td><td style="text-align:right; padding:4px; border-right:1px solid #333;">${fmt(s.qTotal)}</td><td style="text-align:right; padding:4px; border-right:1px solid #333;">${fmt(fe_prime)}</td><td style="text-align:center; padding:4px;">${fmtR(groundRatio)}</td></tr>`;
