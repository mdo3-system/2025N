...
window.PropertyController = {
    ...
    generateBeamHtml: function(beam) {
        ...
        let html = `<div class="calc-box" style="padding:10px; max-height:550px; overflow-y:auto; font-family:sans-serif;">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; background:#fdfafa; padding:8px; border-radius:6px; margin-bottom:12px; border:1px solid #f1e5f5; font-size:11px;">
                ...
                <div style="grid-column: span 2;">
                    <label style="font-weight:bold; color:#7d3c98; display:block; margin-bottom:2px;">連続梁モデル選択</label>
                    <select onchange="window.PropertyController.updateFdProp('beam', ${beam.id}, 'modelType', this.value)" style="width:100%; box-sizing:border-box; padding:4px; border:1px solid #ccc; border-radius:4px; background:#fff;">
                        <option value="both_ends" ${bp.modelType !== 'pillar_supported' ? 'selected' : ''}>両端支点連続梁モデル (ΣN × B)</option>
                        <option value="pillar_supported" ${bp.modelType === 'pillar_supported' ? 'selected' : ''}>柱支点連続梁モデル (ΣN そのまま)</option>
                    </select>
                </div>
            </div>`;
        ...
        if (beam.fdStress && beam.fdStress.pillars && beam.fdStress.pillars.length > 0) {
            ...
            html += this.generateStressDiagramsSvg(beam);
            ...
            let table1 = `<table style="width:100%; border-collapse:collapse; font-size:9px; margin-bottom:12px; border:1px solid #ddd;">
                <thead>
                    <tr style="background:#f2f4f4; border-bottom:1px solid #ddd;">
                        ...
                        <th colspan="4" style="border:1px solid #ddd; padding:3px; text-align:center; background:#ebf5fb;">左加力 (B=${dispB.toFixed(2)})</th>
                        <th colspan="4" style="border:1px solid #ddd; padding:3px; text-align:center; background:#fdf2e9;">右加力 (B=${dispB.toFixed(2)})</th>
                    </tr>
                    <tr style="background:#f2f4f4; border-bottom:1px solid #ddd;">
            ...
            pillars.forEach((p, idx) => {
                ...
                const l_R = (idx === 0 ? seismic.leftward.R_left : (idx === pillars.length - 1 ? seismic.leftward.R_right : 0)).toFixed(3);
                ...
                const r_R = (idx === 0 ? seismic.rightward.R_left : (idx === pillars.length - 1 ? seismic.rightward.R_right : 0)).toFixed(3);
            ...
            spans.forEach((span, sIdx) => {
                ...
                table4 += `<tr>
                    ...
                    <td style="border:1px solid #ddd; padding:3px; text-align:center;">
                        <input type="text" value="${sTopRebar}" onchange="window.PropertyController.updateFdProp('beam_span', ${beam.id}, 'topRebar', this.value, ${sIdx})" style="width:55px; padding:2px; font-size:9px; border:1px solid #ccc; border-radius:3px; text-align:center;">
                    </td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right;">${(span.cap.lMa_top * 1e6 / 195 / span.cap.j || 0).toFixed(1)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:#27ae60;">${span.cap.sMa_top.toFixed(3)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:center;">
                        <input type="text" value="${sBottomRebar}" onchange="window.PropertyController.updateFdProp('beam_span', ${beam.id}, 'bottomRebar', this.value, ${sIdx})" style="width:55px; padding:2px; font-size:9px; border:1px solid #ccc; border-radius:3px; text-align:center;">
                    </td>
            ...
            spans.forEach((span, sIdx) => {
                ...
                table5 += `<tr>
                    ...
                    <td style="border:1px solid #ddd; padding:3px; text-align:center;">
                        <input type="text" value="${sStirrup}" onchange="window.PropertyController.updateFdProp('beam_span', ${beam.id}, 'stirrup', this.value, ${sIdx})" style="width:75px; padding:2px; font-size:9px; border:1px solid #ccc; border-radius:3px; text-align:center;">
                    </td>
    ...
    generateStressDiagramsSvg: function(beam) {
        ...
        pillars.forEach((p, idx) => {
            ...
            svg += `<text x="${px}" y="${y1 + 15}" font-size="8" text-anchor="middle" fill="#64748b" font-weight="bold">${p.name}</text>`;
        ...
        spans.forEach((span, sIdx) => {
            ...
            if (sIdx === 0) {
                svg += `<text x="${xL}" y="${l_yL - 6}" font-size="8" text-anchor="middle" fill="#1d4ed8" font-weight="bold">${l_mL.toFixed(3)}</text>`;
                svg += `<text x="${xL}" y="${r_yL + 12}" font-size="8" text-anchor="middle" fill="#c2410c" font-weight="bold">${r_mL.toFixed(3)}</text>`;
            }
            svg += `<text x="${xR}" y="${l_yR - 6}" font-size="8" text-anchor="middle" fill="#1d4ed8" font-weight="bold">${l_mR.toFixed(3)}</text>`;
            svg += `<text x="${xR}" y="${r_yR + 12}" font-size="8" text-anchor="middle" fill="#c2410c" font-weight="bold">${r_mR.toFixed(3)}</text>`;
        });
        svg += `</g>`;
        ...
        spans.forEach((span, sIdx) => {
            ...
            svg += `<text x="${xMid}" y="${l_y - 4}" font-size="8" text-anchor="middle" fill="#1d4ed8" font-weight="bold">${l_q.toFixed(3)}</text>`;
            svg += `<text x="${xMid}" y="${r_y + 10}" font-size="8" text-anchor="middle" fill="#c2410c" font-weight="bold">${r_q.toFixed(3)}</text>`;
        });
        svg += `</g>`;

        svg += `
            <g transform="translate(${width - 245}, 12)">
                <rect width="235" height="24" rx="4" fill="#ffffff" stroke="#cbd5e1" stroke-width="1" />
                <line x1="10" y1="12" x2="25" y2="12" stroke="#10b981" stroke-width="2" stroke-dasharray="2,2" />
                <text x="30" y="15" font-size="8" fill="#475569" font-weight="bold">長期</text>
                <line x1="75" y1="12" x2="90" y2="12" stroke="#3b82f6" stroke-width="2" />
                <text x="95" y="15" font-size="8" fill="#475569" font-weight="bold">左加力 (短期)</text>
                <line x1="152" y1="12" x2="167" y2="12" stroke="#f97316" stroke-width="2" />
                <text x="172" y="15" font-size="8" fill="#475569" font-weight="bold">右加力 (短期)</text>
            </g>
        `;
