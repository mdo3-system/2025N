...
window.getFoundationSlabReportHtml = function(slab) {
    ...
    html += `<div style="font-weight:bold; color:#2c3e50; border-bottom:1px solid #27ae60; margin-bottom:8px;">📋 検定結果レポート</div>`;
    
    html += `<div style="display:grid; grid-template-columns: 1fr 1fr; gap:5px; margin-bottom:8px;">`;
    html += `<div>接地圧 q: <b>${fmt(s.qTotal)}</b> kN/㎡</div>`;
    ...
    html += `<tr style="background:#eee;"><th style="text-align:left; padding:2px;">項目</th><th style="text-align:right; padding:2px;">必要</th><th style="text-align:right; padding:2px;">許容</th><th style="text-align:center; padding:2px;">判定</th></tr>`;
    html += `<tr><td>短辺 M (kNm)</td><td style="text-align:right;">${fmt(Math.max(s.Mx_center||0, s.Mx_end||0))}</td><td style="text-align:right;">${fmt(s.Ma_short)}</td><td style="text-align:center;">${fmtR(s.ratioShort)}</td></tr>`;
    if (!s.cantileverLength) {
        html += `<tr><td>長辺 M (kNm)</td><td style="text-align:right;">${fmt(Math.max(s.My_center||0, s.My_end||0))}</td><td style="text-align:right;">${fmt(s.Ma_long)}</td><td style="text-align:center;">${fmtR(s.ratioLong)}</td></tr>`;
    }
    html += `</table>`;
    
    if (s.isNG) html += `<div style="color:#e74c3c; font-weight:bold; text-align:center; border:1px solid #e74c3c; padding:2px;">⚠️ 断面不足 (NG)</div>`;
    else html += `<div style="color:#27ae60; font-weight:bold; text-align:center; border:1px solid #27ae60; padding:2px;">✅ 安全 (OK)</div>`;
