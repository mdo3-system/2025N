    ...
    <div style="font-family:'Hiragino Kaku Gothic ProN','Meiryo',sans-serif;font-size:12px;line-height:1.5;padding:4px;">
        ...
        <table style="width:100%;border-collapse:collapse;border:1px solid #bdc3c7;">
            ...
            <tbody>
                ...
                ${row('負担幅 (m)', tw, tw)}
                ${row('線荷重 w (kN/m)', wL, '—')}
                ${row('最大曲げ M (kN·m)', (s.stressData ? fmt(Math.max(s.stressData.M_long_mid_Nmm, s.stressData.M_long_end_Nmm) / 1e6, 2) : '—'), (s.stressData ? fmt((Math.max(s.stressData.M_long_mid_Nmm, s.stressData.M_long_end_Nmm) + (s.stressData.M_short_end_Nmm || 0)) / 1e6, 2) : '—'))}
                ${row('最大せん断 Q (kN)', (s.stressData ? fmt(s.stressData.Q_long_N / 1e3, 2) : '—'), (s.stressData ? fmt(((s.stressData.Q_long_N || 0) + (s.stressData.Qe_N || 0)) / 1e3, 2) : '—'))}
                ${sep('✅ 許容耐力・検定')}
