...
window.FoundationEngine = {
        ...
        (slabs || []).forEach(slab => {
            ...
            for (let i = 0; i < edges.length; i++) {
                ...
                if (area > 100) {
                    ...
                    slab.tributaryPolygons.push(trib);Leverage
    ...
    getBeamPillars: function(beam, state) {
        ...
        if (!hasStart) pillarsWithX.push({ id: 'support_p1', name: '支点1', x: 0, seismicAxial: 0 });
        if (!hasEnd) pillarsWithX.push({ id: 'support_p2', name: '支点2', x: L_beam / 1000, seismicAxial: 0 });
    ...
    runFoundationBeamAnalysis: function(beams, slabs, state) {
        ...
        beams.forEach(beam => {
            ...
            for (let i = 0; i < pillars.length - 1; i++) {
                ...
                const n_coef = 2.0;
                const M_combo_L_left = M_end + seismic.leftward.Mf[i];
                const M_combo_L_right = M_end + seismic.leftward.Mf[i + 1];
                const Q_combo_L = Q_L + n_coef * seismic.leftward.Qe[i];

                const M_combo_R_left = M_end + seismic.rightward.Mf[i];
                const M_combo_R_right = M_end + seismic.rightward.Mf[i + 1];
                ...
                const topRebar = this.parseRebar(p.topRebar || '1-D13');
                const botRebar = this.parseRebar(p.bottomRebar || '1-D13');
                const stRebar  = this.parseStirrups(p.stirrup || '1-D10@200');

                const topSteel = this.getSteelStrength(p.topRebar || '1-D13');
                const botSteel = this.getSteelStrength(p.bottomRebar || '1-D13');
                const stSteel  = this.getSteelStrength(p.stirrup || '1-D10@200');
