...
window.PropertyController = {
    ...
    generateBeamHtml: function(beam) {
        ...
        let html = `<div class="calc-box" style="padding:10px; max-height:550px; overflow-y:auto; font-family:sans-serif;">
            <div style="font-size:12px; font-weight:bold; color:#2c3e50; border-bottom:2px solid #8e44ad; margin-bottom:10px; padding-bottom:5px;">🏗️ 基礎梁 共通設定</div>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; background:#fdfafa; padding:8px; border-radius:6px; margin-bottom:12px; border:1px solid #f1e5f5; font-size:11px;">
                <div>
                    <label style="font-weight:bold; color:#7d3c98; display:block; margin-bottom:2px;">符号</label>
                    <input type="text" value="${bp.symbol || 'FG1'}" onchange="window.PropertyController.updateFdProp('beam', ${beam.id}, 'symbol', this.value)" style="width:100%; box-sizing:border-box; padding:4px; border:1px solid #ccc; border-radius:4px;">
                </div>
                <div>
                    <label style="font-weight:bold; color:#7d3c98; display:block; margin-bottom:2px;">梁幅 W(mm)</label>
                    <input type="number" value="${bp.width || 150}" onchange="window.PropertyController.updateFdProp('beam', ${beam.id}, 'width', this.value)" style="width:100%; box-sizing:border-box; padding:4px; border:1px solid #ccc; border-radius:4px;">
                </div>
                <div>
                    <label style="font-weight:bold; color:#7d3c98; display:block; margin-bottom:2px;">梁成 H(mm)</label>
                    <input type="number" value="${bp.height || 640}" onchange="window.PropertyController.updateFdProp('beam', ${beam.id}, 'height', this.value)" style="width:100%; box-sizing:border-box; padding:4px; border:1px solid #ccc; border-radius:4px;">
                </div>
                <div>
                    <label style="font-weight:bold; color:#7d3c98; display:block; margin-bottom:2px;">根入深さ (mm)</label>
                    <input type="number" value="${bp.embedDepth || 250}" onchange="window.PropertyController.updateFdProp('beam', ${beam.id}, 'embedDepth', this.value)" style="width:100%; box-sizing:border-box; padding:4px; border:1px solid #ccc; border-radius:4px;">
                </div>
                <div>
                    <label style="font-weight:bold; color:#7d3c98; display:block; margin-bottom:2px;">上主筋</label>
                    <input type="text" value="${bp.topRebar || '1-D13'}" onchange="window.PropertyController.updateFdProp('beam', ${beam.id}, 'topRebar', this.value)" style="width:100%; box-sizing:border-box; padding:4px; border:1px solid #ccc; border-radius:4px;">
                </div>
                <div>
                    <label style="font-weight:bold; color:#7d3c98; display:block; margin-bottom:2px;">下主筋</label>
                    <input type="text" value="${bp.bottomRebar || '1-D13'}" onchange="window.PropertyController.updateFdProp('beam', ${beam.id}, 'bottomRebar', this.value)" style="width:100%; box-sizing:border-box; padding:4px; border:1px solid #ccc; border-radius:4px;">
                </div>
                <div style="grid-column: span 2;">
                    <label style="font-weight:bold; color:#7d3c98; display:block; margin-bottom:2px;">ST筋 (あばら筋)</label>
                    <input type="text" value="${bp.stirrup || '1-D10@200'}" onchange="window.PropertyController.updateFdProp('beam', ${beam.id}, 'stirrup', this.value)" style="width:100%; box-sizing:border-box; padding:4px; border:1px solid #ccc; border-radius:4px;">
                </div>
            </div>`;
        ...
        if (beam.fdStress && beam.fdStress.pillars && beam.fdStress.pillars.length > 0) {
            ...
            let table1 = `<table style="width:100%; border-collapse:collapse; font-size:9px; margin-bottom:12px; border:1px solid #ddd;">
                <thead>
                    <tr style="background:#f2f4f4; border-bottom:1px solid #ddd;">
                        ...
                        <th colspan="4" style="border:1px solid #ddd; padding:3px; text-align:center; background:#ebf5fb;">左加力 (B=0.5)</th>
                        <th colspan="4" style="border:1px solid #ddd; padding:3px; text-align:center; background:#fdf2e9;">右加力 (B=0.5)</th>
                    </tr>
                    <tr style="background:#f2f4f4; border-bottom:1px solid #ddd;">
            ...
            pillars.forEach((p, idx) => {
                ...
                table1 += `<tr>
                    <td style="border:1px solid #ddd; padding:3px; font-weight:bold;">${p.name}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right;">${p.x.toFixed(2)}</td>
            ...
            spans.forEach(span => {
                table2 += `<tr>
                    <td style="border:1px solid #ddd; padding:3px; font-weight:bold;">${span.spanName}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right;">${span.L.toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right;">${span.sigma_e.toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right;">${span.B_trib.toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:#27ae60;">${span.M_mid.toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:#2980b9;">${span.M_end.toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:#7d3c98;">${span.Q_L.toFixed(2)}</td>
                </tr>`;
            });
            table2 += `</tbody></table>`;
            html += table2;
            ...
            spans.forEach(span => {
                table3 += `<tr>
                    <td style="border:1px solid #ddd; padding:3px; font-weight:bold;">${span.spanName}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; color:#2980b9;">${span.leftward.M_left.toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; color:#27ae60;">${span.leftward.M_right.toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:#7d3c98;">${span.leftward.Q.toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; color:#c0392b;">${span.rightward.M_left.toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; color:#e67e22;">${span.rightward.M_right.toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:#e74c3c;">${span.rightward.Q.toFixed(2)}</td>
                </tr>`;
            });
            table3 += `</tbody></table>`;
            html += table3;
            ...
            let table4 = `<table style="width:100%; border-collapse:collapse; font-size:9px; margin-bottom:12px; border:1px solid #ddd;">
                <thead>
                    <tr style="background:#f2f4f4; border-bottom:1px solid #ddd;">
                        ...
                        <th colspan="3" style="border:1px solid #ddd; padding:3px; text-align:center;">上端主筋</th>
                        <th colspan="3" style="border:1px solid #ddd; padding:3px; text-align:center;">下端主筋</th>
                    </tr>
                    <tr style="background:#f2f4f4; border-bottom:1px solid #ddd;">
                        <th style="border:1px solid #ddd; padding:2px;">鉄筋</th>
                        <th style="border:1px solid #ddd; padding:2px;">at(㎟)</th>
                        <th style="border:1px solid #ddd; padding:2px;">sMa(kNm)</th>
                        <th style="border:1px solid #ddd; padding:2px;">鉄筋</th>
                        <th style="border:1px solid #ddd; padding:2px;">at(㎟)</th>
                        <th style="border:1px solid #ddd; padding:2px;">lMa(kNm)</th>
                    </tr>
                </thead>
                <tbody>`;
            ...
            spans.forEach(span => {
                table4 += `<tr>
                    <td style="border:1px solid #ddd; padding:3px; font-weight:bold;">${span.spanName}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right;">${span.cap.h}</td>
                    <td style="border:1px solid #ddd; padding:3px; font-weight:bold;">${bp.topRebar || '1-D13'}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right;">${(span.cap.lMa_top * 1e6 / 195 / span.cap.j || 0).toFixed(1)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:#27ae60;">${span.cap.sMa_top.toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; font-weight:bold;">${bp.bottomRebar || '1-D13'}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right;">${(span.cap.lMa_bot * 1e6 / 195 / span.cap.j || 0).toFixed(1)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:#2980b9;">${span.cap.lMa_bot.toFixed(2)}</td>
                </tr>`;
            });
            table4 += `</tbody></table>`;
            html += table4;
            ...
            let table5 = `<table style="width:100%; border-collapse:collapse; font-size:9px; margin-bottom:12px; border:1px solid #ddd;">
                <thead>
                    <tr style="background:#f2f4f4; border-bottom:1px solid #ddd;">
                        <th style="border:1px solid #ddd; padding:3px;">柱間</th>
                        <th style="border:1px solid #ddd; padding:3px;">幅 b</th>
                        <th style="border:1px solid #ddd; padding:3px;">ST筋</th>
                        <th style="border:1px solid #ddd; padding:3px;">pw</th>
                        <th style="border:1px solid #ddd; padding:3px;">長期Qa(kN)</th>
                        <th style="border:1px solid #ddd; padding:3px;">短期Qa_L(kN)</th>
                        <th style="border:1px solid #ddd; padding:3px;">短期Qa_R(kN)</th>
                    </tr>
                </thead>
                <tbody>`;
            ...
            spans.forEach(span => {
                table5 += `<tr>
                    <td style="border:1px solid #ddd; padding:3px; font-weight:bold;">${span.spanName}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right;">${span.cap.b}</td>
                    <td style="border:1px solid #ddd; padding:3px; font-weight:bold;">${bp.stirrup || '1-D10@200'}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right;">${span.cap.pw.toFixed(5)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:#27ae60;">${span.cap.lQa.toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:#2980b9;">${span.cap.sQa_L.toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:#7d3c98;">${span.cap.sQa_R.toFixed(2)}</td>
                </tr>`;
            });
            table5 += `</tbody></table>`;
            html += table5;
            ...
            spans.forEach(span => {
                ...
                table6 += `<tr>
                    <td style="border:1px solid #ddd; padding:3px; font-weight:bold;">${span.spanName}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:${span.rM_L > 1.0 ? 'red' : '#2980b9'}">${span.rM_L.toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:${span.rQ_L > 1.0 ? 'red' : '#27ae60'}">${span.rQ_L.toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:${Math.max(span.leftward.rM_left, span.leftward.rM_right) > 1.0 ? 'red' : '#7d3c98'}">${Math.max(span.leftward.rM_left, span.leftward.rM_right).toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:${span.leftward.rQ > 1.0 ? 'red' : '#138d75'}">${span.leftward.rQ.toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:${Math.max(span.rightward.rM_left, span.rightward.rM_right) > 1.0 ? 'red' : '#d35400'}">${Math.max(span.rightward.rM_left, span.rightward.rM_right).toFixed(2)}</td>
                    <td style="border:1px solid #ddd; padding:3px; text-align:right; font-weight:bold; color:${span.rightward.rQ > 1.0 ? 'red' : '#900c3f'}">${span.rightward.rQ.toFixed(2)}</td>
    ...
    generateSlabHtml: function(item) {
        ...
        return `
            <div class="calc-box" style="padding:10px; max-height:600px; overflow-y:auto; font-family:sans-serif;">
                <div style="font-size:12px; font-weight:bold; color:#2c3e50; border-bottom:2px solid #27ae60; margin-bottom:10px; padding-bottom:5px;">📐 スラブ設定・検定</div>
                
                <div class="calc-row" style="margin-bottom:8px;">
                    <label style="width:80px; display:inline-block; font-size:11px;">符号 (スラブ名)</label>
                    <input type="text" value="${p.name || 'S1'}" onchange="window.PropertyController.updateFdProp('slab', ${item.id}, 'name', this.value)" style="width:100px;">
                </div>

                <div class="calc-row" style="margin-bottom:8px;">
                    <label style="width:80px; display:inline-block; font-size:11px;">支持条件</label>
                    <select onchange="window.PropertyController.updateFdProp('slab', ${item.id}, 'support', this.value)" style="width:180px;">
                ...
                ${p.support === '片持ち' ? `
                <div class="calc-row" style="margin-bottom:8px;">
                    <label style="width:80px; display:inline-block; font-size:11px;">片持ち長さ</label>
                    <input type="number" step="0.1" value="${p.cantileverLength || 0.9}" onchange="window.PropertyController.updateFdProp('slab', ${item.id}, 'cantileverLength', this.value)" style="width:60px;"> m
                </div>
                ` : ''}

                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; margin-bottom:12px; background:#f9f9f9; padding:8px; border-radius:4px;">
                    <div>
                        <div style="font-size:10px; color:#666;">厚さ(mm)</div>
                        <input type="number" value="${p.slabThickness || 150}" onchange="window.PropertyController.updateFdProp('slab', ${item.id}, 'slabThickness', this.value)" style="width:60px;">
                    </div>
                    <div>
                        <div style="font-size:10px; color:#666;">かぶり(mm)</div>
                        <input type="number" value="${p.coverDepth || 70}" onchange="window.PropertyController.updateFdProp('slab', ${item.id}, 'coverDepth', this.value)" style="width:60px;">
                    </div>
                </div>

                <div style="margin-bottom:12px;">
                    <div style="font-size:11px; font-weight:bold; color:#27ae60; margin-bottom:5px;">⛓️ 配筋設定</div>
                    <table style="width:100%; font-size:10px; border-collapse:collapse;">
                        <tr>
                            <td>短辺方向</td>
                            <td>
                                <select onchange="window.PropertyController.updateFdProp('slab', ${item.id}, 'rebarShort.type', this.value)">
                                    ${rebars.map(r => `<option value="${r}" ${p.rebarShort?.type === r ? 'selected' : ''}>${r}</option>`).join('')}
                                </select>
                            </td>
                            <td>@</td>
                            <td><input type="number" value="${p.rebarShort?.pitch || 200}" onchange="window.PropertyController.updateFdProp('slab', ${item.id}, 'rebarShort.pitch', this.value)" style="width:45px;"></td>
                        </tr>
                        <tr>
                            <td>長辺方向</td>
                            <td>
                                <select onchange="window.PropertyController.updateFdProp('slab', ${item.id}, 'rebarLong.type', this.value)">
                                    ${rebars.map(r => `<option value="${r}" ${p.rebarLong?.type === r ? 'selected' : ''}>${r}</option>`).join('')}
                                </select>
                            </td>
                            <td>@</td>
                            <td><input type="number" value="${p.rebarLong?.pitch || 200}" onchange="window.PropertyController.updateFdProp('slab', ${item.id}, 'rebarLong.pitch', this.value)" style="width:45px;"></td>
                        </tr>
                    </table>
                </div>

                <div id="slab-calc-result-container" style="margin-top:15px; border-top:1px dashed #ccc; padding-top:10px;">
...
window.getFoundationSlabReportHtml = function(slab) {
    ...
    html += `<tr style="border-bottom:1px solid #ddd;"><td style="padding:4px; border-right:1px solid #333;">接地圧 σe / fe'<br><small style="font-size:8px;color:#666;">fe' = fe - 24*d</small></td><td style="text-align:right; padding:4px; border-right:1px solid #333;">${fmt(s.qTotal)}</td><td style="text-align:right; padding:4px; border-right:1px solid #333;">${fmt(fe_prime)}<br><small style="font-size:8px;color:#666;">(${fe} - 24*${d_val})</small></td><td style="text-align:center; padding:4px;">${fmtR(groundRatio)}</td></tr>`;
    
    // 曲げモーメント判定
    html += `<tr style="border-bottom:1px solid #ddd;"><td style="padding:4px; border-right:1px solid #333;">短辺 M (kNm)</td><td style="text-align:right; padding:4px; border-right:1px solid #333;">${fmt(Math.max(s.Mx_center||0, s.Mx_end||0))}</td><td style="text-align:right; padding:4px; border-right:1px solid #333;">${fmt(s.Ma_short)}</td><td style="text-align:center; padding:4px;">${fmtR(s.ratioShort)}</td></tr>`;
    if (!s.cantileverLength) {
        html += `<tr><td style="padding:4px; border-right:1px solid #333;">長辺 M (kNm)</td><td style="text-align:right; padding:4px; border-right:1px solid #333;">${fmt(Math.max(s.My_center||0, s.My_end||0))}</td><td style="text-align:right; padding:4px; border-right:1px solid #333;">${fmt(s.Ma_long)}</td><td style="text-align:center; padding:4px;">${fmtR(s.ratioLong)}</td></tr>`;
    }
    html += `</table>`;
