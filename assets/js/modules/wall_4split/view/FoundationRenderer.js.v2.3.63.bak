/**
 * view/FoundationRenderer.js - 基礎レイヤー描画エンジン
 * v2.3.0 リファクタリング
 */
...
window.FoundationRenderer = {
    ...
    drawSlabs: function(state, toCanvas, fdSel) {
        ...
        (state.foundationSlabs || []).filter(s => !state.elementVisibility || state.elementVisibility.f_slabs !== false).forEach((slab, si) => {
            ...
            if (lp.cx != null) {
                ...
                if (state.averageGroundPressure) {
                    ctx.font = 'bold 12px sans-serif'; ctx.fillStyle = '#e67e22';
                    ctx.fillText(`${state.averageGroundPressure.toFixed(2)} kN/m2`, lp.cx, lp.cy + 10);
                }
            }
            ctx.restore();
        });
    ...
    drawTributary: function(state, toCanvas) {
        if (!state.elementVisibility.f_slabs) return;
        const ctx = state.ctx;
        (state.foundationSlabs || []).forEach(slab => {
            ...
            slab.tributaryPolygons.forEach((tribEntry, i) => {
                ...
                ctx.save();
                ctx.setLineDash([4, 4]); ctx.strokeStyle = '#7f8c8d'; ctx.lineWidth = 1;
                ctx.beginPath();
                poly.forEach((v, idx) => {
                    const p = toCanvas(v, null);
                ...
                const pC = toCanvas(cxSum, cySum);
                if (pC.cx != null) {
                    let areaA = tribEntry.area / 1e6 || 0;
                    let widthB = tribEntry.width || 0;
                    ctx.font = 'bold 11px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
                    ctx.strokeStyle = '#ffffff'; ctx.lineWidth = 3;
                    ctx.strokeText(`A = ${areaA.toFixed(2)} ㎡`, pC.cx, pC.cy - 7);
                    ctx.fillStyle = '#2980b9'; ctx.fillText(`A = ${areaA.toFixed(2)} ㎡`, pC.cx, pC.cy - 7);
                    if (widthB > 0) {
                        ctx.strokeText(`B = ${widthB.toFixed(2)} m`, pC.cx, pC.cy + 7);
                        ctx.fillText(`B = ${widthB.toFixed(2)} m`, pC.cx, pC.cy + 7);
                    }
                }
                ctx.restore();
            });
    ...
    drawExteriorWalls: function(state, toCanvas, fdSel) {
        if (!state.elementVisibility.f_ext_walls) return;
        const ctx = state.ctx;
        (state.exteriorWalls || []).filter(ew => ew.floor === state.currentFloor).forEach(ew => {
            ...
            ctx.strokeStyle = isSelected ? '#ff00ff' : '#e67e22';
            ctx.lineWidth = isSelected ? 8 : 3;
            ctx.setLineDash(isSelected ? [] : [12, 6]);
            ctx.beginPath();
    ...
    drawManholes: function(state, toCanvas, fdSel) {
        if (!state.elementVisibility.f_manholes) return;
        const ctx = state.ctx;
    ...
    drawPreviews: function(state, toCanvas) {
        ...
        if ((fm === 'f_ext_wall' || fm === 'f_slab') && fdPts.length > 0) {
            ctx.save(); ctx.strokeStyle = fm === 'f_ext_wall' ? '#e67e22' : '#2980b9';
