...
function resizeCanvas() {
    const canvas = window.canvas || (window.AppState ? window.AppState.canvas : null);
    if (!canvas) return;
...
function draw() {
    ...
    const _appMode = window.AppState ? (window.AppState.currentAppMode || 'wall') : 'wall';
    ...
    if (modeStr !== 'area') {
        ...
        walls.filter(w => w.floor === currentFloor && window.AppState.elementVisibility.walls).forEach(w => {
            ...
            if (p1.cx != null && !isNaN(p1.cx) && p2.cx != null && !isNaN(p2.cx)) {
                ...
                if (!mark && (w.totalVal - (w.braceVal || 0) > 0)) {
...
function drawFoundationLayer(toCanvas) {
    ...
    // ---- 作図中プレビュー ----
    const fm = window.AppState ? (window.AppState.foundationMode || 'f_beam') : 'f_beam';
