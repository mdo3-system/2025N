...
// --- 基礎モード関連の操作ハンドラは controllers/FoundationController.js に移行されました ---
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        bC('tab-fd', () => switchAppMode('foundation'));
        ...
        if (canvas) {
            ...
            canvas.addEventListener('mousedown', (e) => {
                ...
                if ((window.AppState ? window.AppState.currentAppMode : 'wall') === 'foundation' && e.button === 0) {
                    handleFoundationMouseDown(e);
                    return;
                }

                if (e.button === 1 || e.button === 2 || (e.button === 0 && !hoveredPillar && m !== 'add-grid' && m !== 'del-wall' && m !== 'add-pillar' && m !== 'edit-wall' && m !== 'edit-text' && m !== 'del-grid' && m !== 'draw-area')) { isDragging = true; lastMouseX = e.clientX; lastMouseY = e.clientY; return; }
                if (e.button === 0) {
                    if ((m === 'wall' || m === 'window') && hoveredPillar) {
                        if (!selectedPillar) { selectedPillar = hoveredPillar; if (m === 'wall') showPillarProps(hoveredPillar); }
                            ...
                            if (m === 'wall') {
                                ...
                                let nw = { id: Date.now(), p1: selectedPillar, p2: hoveredPillar, length: l, totalVal: sumVal, outPanelVal: p1Val, inPanelVal: p2Val, braceVal: bVal, outPanelName: outName, inPanelName: inName, isTasuki: t, braceName: braceName, floor: currentFloor };
            ...
            canvas.addEventListener('mousemove', (e) => {
                ...
                if ((window.AppState ? window.AppState.currentAppMode : 'wall') === 'foundation') {
        ...
        window.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'Escape' && (window.AppState ? window.AppState.currentAppMode : 'wall') === 'foundation') {
                fdDrawPoints = []; fdSelectedPillarLike = null;
                requestAnimationFrame(draw);
                return;
            }
