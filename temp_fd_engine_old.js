...
window.FoundationEngine = {
    COEFFS: {
        '4霎ｺ蝗ｺ螳・:                     { mcx: 0.025, max: 0.045, mcy: 0.015, may: 0.035 },
        '3霎ｺ蝗ｺ螳・霎ｺ繝斐Φ・磯聞霎ｺ繝斐Φ・・:  { mcx: 0.035, max: 0.055, mcy: 0.020, may: 0.040 },
        '3霎ｺ蝗ｺ螳・霎ｺ繝斐Φ・育洒霎ｺ繝斐Φ・・:  { mcx: 0.040, max: 0.065, mcy: 0.025, may: 0.050 },
        '2髫｣霎ｺ蝗ｺ螳・髫｣霎ｺ繝斐Φ':          { mcx: 0.045, max: 0.075, mcy: 0.035, may: 0.065 },
        '髟ｷ霎ｺ2霎ｺ蝗ｺ螳夂洒霎ｺ2霎ｺ繝斐Φ':      { mcx: 0.060, max: 0.090, mcy: 0.030, may: 0.000 },
        '遏ｭ霎ｺ2霎ｺ蝗ｺ螳夐聞霎ｺ2霎ｺ繝斐Φ':      { mcx: 0.030, max: 0.000, mcy: 0.060, may: 0.090 },
        '1霎ｺ蝗ｺ螳・霎ｺ繝斐Φ・磯聞霎ｺ蝗ｺ螳夲ｼ・:  { mcx: 0.065, max: 0.100, mcy: 0.040, may: 0.000 },
        '1霎ｺ蝗ｺ螳・霎ｺ繝斐Φ・育洒霎ｺ蝗ｺ螳夲ｼ・:  { mcx: 0.050, max: 0.000, mcy: 0.075, may: 0.110 },
        '4霎ｺ繝斐Φ':                     { mcx: 0.080, max: 0.000, mcy: 0.050, may: 0.000 }
    },
    ...
    runAnalysis: function(state) {
        ...
        console.log(`女・・[${window.APP_VERSION || 'v2.4.2'}] Running Foundation Analysis...`);
        ...
        console.log("笨・Foundation Analysis Complete.");
    },
    ...
    updateGroundPressure: function(state) {
        ...
        // 蜷・嚴縺ｮ螳溷､門｣∫ｷ夲ｼ・.exteriorWalls・峨・邱丞ｻｶ髟ｷ・・・峨ｒ邂怜・
        let len1F = 0, len2F = 0;
        ...
        // 螳溷､門｣・擇遨阪∵悴菴懷峙縺ｮ蝣ｴ蜷医・蠕捺擂繝・ヵ繧ｩ繝ｫ繝・(a1 + a2) * 1.0 縺ｫ繝輔か繝ｼ繝ｫ繝舌ャ繧ｯ
    ...
    calculateSlabAnalysis: function(slabs, avgBuildingPressure) {
        ...
        slabs.forEach(slab => {
            ...
            // --- 逕ｻ蜒上Ο繧ｸ繝・け縺ｫ蝓ｺ縺･縺城寔險・---
            
            // (1) 霆ｸ蜉・(kN) 髮・ｨ・- 讓呎ｺ也ｭ牙・蟶・ｻｺ迚ｩ闕ｷ驥阪Δ繝・Ν縺ｫ蝓ｺ縺･縺肴紛蜷・            const axialPressure = s.averageBuildingPressure || 3.67;
            const totalAxial_kN = area * axialPressure;

            // (2) 遶倶ｸ翫ｊ驥埼㍼ (kN) 髮・ｨ・            let stemWeight_kN = 0;
            ...
            // (3) 蝮・＠闕ｷ驥・(kN/m2) 縺ｮ讒区・隕∫ｴ
            const D_val = p.slabThickness || 150;
            const topH_val = p.slabTopHeight || 50;
            const slabMatWeight = 0; // 逕ｻ蜒上♀繧医・繝ｦ繝ｼ繧ｶ繝ｼ縺ｮ縺碑ｦ∵悍縺ｫ蝓ｺ縺･縺阪せ繝ｩ繝冶・驥阪・諡ｾ繧上↑縺・            
            const stemPressure = area > 0 ? (stemWeight_kN / area) : 0;
            
            // 蝮・＠闕ｷ驥・= (霆ｸ蜉・髱｢遨・ + (遶倶ｸ翫ｊ/髱｢遨・ + 繧ｹ繝ｩ繝冶・驥・            const deadLoad = axialPressure + stemPressure + slabMatWeight;

            // (4) 蠎願差驥・(kN/m2)
            // 遨崎ｼ芽差驥・1.30 + 莉穂ｸ翫￡ 0.44 = 1.74 (菴丞ｮ・
            ...
            if (p.support === '迚・戟縺｡') {
                ...
                slab.fdStress = { 
                    ...
                    ratioShort: Mx / (Ma_short || 1), supportName: '迚・戟縺｡',
                    isNG: (Mx / (Ma_short || 1) > 1.0) 
                };
            } else {
                const coeffs = this.COEFFS[p.support] || this.COEFFS['4霎ｺ蝗ｺ螳・];
    ...
    _isBeamOnSlabBoundary: function(b, poly) {
        const tol = 50;
        // 譴√・荳｡遶ｯ縺後・繝ｪ繧ｴ繝ｳ縺ｮ蠅・阜荳翫↓縺ゅｋ縺九√°縺､譴√・荳ｭ轤ｹ縺悟｢・阜荳翫↓縺ゅｋ縺・        if (!this._isPointOnBoundary(b.p1, poly) || !this._isPointOnBoundary(b.p2, poly)) return false;
    ...
    getBeamPillars: function(beam, state) {
        ...
        const nameStart = window.getGridNameAt ? window.getGridNameAt(p1.x, p1.y) : '謾ｯ轤ｹ1';
        const nameEnd = window.getGridNameAt ? window.getGridNameAt(p2.x, p2.y) : '謾ｯ轤ｹ2';
    ...
    runFoundationBeamAnalysis: function(beams, slabs, state) {
        ...
        beams.forEach(beam => {
            ...
            for (let i = 0; i < pillars.length - 1; i++) {
                ...
                // Calculate span-specific slab load, ground pressure, and tributary width
                const triMult = s.config?.triangleMultiplier || 1.33;
                let spanSlabArea = 0;
                let spanSlabWidth = 0;
                let spanSlabLoad_kN = 0;
                ...
                (slabs || []).forEach(slab => {
                    ...
                    (slab.tributaryPolygons || []).forEach(tp => {
                        if (tp.beamId === beam.id) {
                            ...
                            // Span endpoints in mm
                            const x1_mm = p1.globalX;
                            const y1_mm = p1.globalY;
                            const x2_mm = p2.globalX;
                            const y2_mm = p2.globalY;
                            
                            const dist = this._distToSegment(mx, my, {x: x1_mm, y: y1_mm}, {x: x2_mm, y: y2_mm});

                            if (dist < 150) { // 150mm tolerance
                                const isTriangle = window.FoundationEngine._isTriangle(tp.polygon);
                                const factor = isTriangle ? triMult : 1.0;
                                spanSlabArea += tp.area / 1e6;
                                spanSlabWidth += tp.width;
                                spanSlabLoad_kN += qSlab * (tp.area / 1e6) * factor;
                            }
                        }
                    });
                });

                const spanSigma_e = spanSlabWidth > 0 ? (spanSlabLoad_kN / spanSlabWidth / spanL) : s.averageGroundPressure || 12.0;
                const spanB_trib = spanSlabWidth;
                const spanW = (spanL > 0 ? (spanSlabLoad_kN / spanL) : 0) + w_self;
    ...
    _isTriangle: function(poly) {
        ...
        for (let k = 0; k < poly.length; k++) {
            ...
            if (lenU > 1.0 && lenV > 1.0) {
                ...
                if (sinTheta > 0.087) { // 5 degrees or more bend is a real corner
                    nonCollinear++;
                }
            }
        }
        return nonCollinear === 3;
    }
};
