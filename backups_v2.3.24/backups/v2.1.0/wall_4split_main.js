...
// ※ 状態のブリッジ処理は wall_4split_bridge.js へ移行しました。
    ...
    reader.onload = function (ev) {
        try {
            ...
            if (d.layerVisibility) {
                appLayerVisibility = d.layerVisibility;
            }
            ...
            bgLinesOriginal.forEach(e => {
                ...
                if (!(e.layer in appLayerVisibility)) appLayerVisibility[e.layer] = true;
            });
            bgTextsOriginal.forEach(t => {
                if (!t.layer) t.layer = 'BG_UNASSIGNED';
                if (!(t.layer in appLayerVisibility)) appLayerVisibility[t.layer] = true;
            });
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        // [v2.1.1] Initialize Application Controllers
        window.InputController.init();
        window.InputController.initCanvas(canvas);
...
// モーダル内のフィールドを動的に更新する関数
window.updateModalFields = function(type) {
    const fieldsContainer = document.getElementById("dynamic-fields");
    if (!fieldsContainer || !selectedElement) return;
    ...
    if (type === "wall") {
        html = `
            <div class="prop-edit-row">
                ...
                <select id="edit-wall-p1">
                    ${(window.getMasterWallList ? window.getMasterWallList() : []).map(o => {
                        const isSel = item.outPanelId ? (item.outPanelId === o.id) : (item.outPanelName || "").trim() === (o.text || "").trim();
                        return `<option value="${o.id}" data-val="${o.val}" ${isSel ? "selected" : ""}>${o.text}</option>`;
                    }).join("")}
                </select>
            </div>
            <div class="prop-edit-row">
                ...
                <select id="edit-wall-p2">
                    ${(window.getMasterWallList ? window.getMasterWallList() : []).map(o => {
                        const isSel = item.inPanelId ? (item.inPanelId === o.id) : (item.inPanelName || "").trim() === (o.text || "").trim();
                        return `<option value="${o.id}" data-val="${o.val}" ${isSel ? "selected" : ""}>${o.text}</option>`;
                    }).join("")}
                </select>
            </div>
            <div class="prop-edit-row">
                ...
                <select id="edit-wall-b">
                    ${(window.getMasterBraceList ? window.getMasterBraceList() : []).map(o => {
                        const isSel = item.braceId ? (item.braceId === o.id) : (item.braceName || "").trim() === (o.text || "").trim();
                        return `<option value="${o.id}" ${isSel ? "selected" : ""}>${o.text}</option>`;
                    }).join("")}
                </select>
            </div>
        `;
        ...
        html = `
            <div class="prop-edit-row">
                <label>幅 (mm)</label>
                <input type="number" id="edit-win-width" value="${item.length || 910}">
            </div>
        `;
    }
...
window.openPropertyModal = function(hit) {
    ...
    if (type === "pillar") {
        ...
        html = `
            <div class="prop-edit-row">
                <label>要素種別</label>
                <select id="edit-element-type" onchange="window.updateModalFields(this.value)">
                    <option value="wall" ${type === "wall" ? "selected" : ""}>耐力壁</option>
                    <option value="window" ${type === "window" ? "selected" : ""}>開口部</option>
                </select>
            </div>
            <div id="dynamic-fields"></div>
        `;
        body.innerHTML = html;
        window.updateModalFields(type);
    } else if (type === "area") {
        html = `
            <div class="prop-edit-row">
                <label>面積の種類</label>
...
window.applyPropertyChanges = function() {
    ...
    if (oldType === "pillar") {
        ...
        if (oldType !== newType) {
            if (newType === "window") {
                // 壁 -> 開口
                walls = walls.filter(w => w !== item);
                windowsArr.push(item);
                selectedElement.type = "window";
            } else {
                // 開口 -> 壁
                windowsArr = windowsArr.filter(w => w !== item);
                walls.push(item);
        ...
        if (newType === "wall") {
            ...
            // 仕様マスターから情報を「呼び出す」形式
            item.outPanelId = p1.value;
            let p1Spec = window.getWallSpec(p1.value);
            item.outPanelVal = p1Spec.val;
            item.outPanelName = p1Spec.text;

            item.inPanelId = p2.value;
            let p2Spec = window.getWallSpec(p2.value);
            item.inPanelVal = p2Spec.val;
            item.inPanelName = p2Spec.text;

            item.braceId = b.value;
            let bSpec = window.getBraceSpec(b.value);
            item.braceVal = bSpec.val;
            item.braceName = bSpec.text;

            item.isTasuki = item.braceName.includes('たすき');
