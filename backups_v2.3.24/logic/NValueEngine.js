...
window.NValueEngine = {
    ...
    calculateNValues: function(state) {
        ...
        const h1 = (typeof window.getVal === 'function') ? window.getVal('n-h1') : (config.floorHeight1F || 2.7);
        const h2 = (typeof window.getVal === 'function') ? window.getVal('n-h2') : (config.floorHeight2F || 2.7);
        ...
        ['2F', '1F'].forEach(f => {
            s.pillars.filter(p => !p.isDeleted && !p.isInvalidPos && p.floor === f).forEach(p => {
                ...
                const p_d = p.manualD || (f === '1F' ? ((typeof window.getVal === 'function') ? window.getVal('p-d1') : (config.pillarDepth1F || 105)) : ((typeof window.getVal === 'function') ? window.getVal('p-d2') : (config.pillarDepth2F || 105)));
