...
// createLayerFilteredImage(), createNativeCanvasImage(), createHighResPlanImage(),
// showAreaPreview(), window.buildWallLegendData, async generateDoc()
// データは cad.js / render.js / calc.js のグローバル変数から参照します。
// ==========================================
...
function createLayerFilteredImage(docType, targetLayers, bgLayers, floorStr = null, showAreaDims = true, dimScale = 1.0, isPrint = false) {
    const data = docDrawings[docType];
    // ★ 修正1: DXF未読み込みでも、draw-area で作図した areaLines があれば続行する
    const hasManualAreas = floorStr && areaLines.some(a => a.floor === floorStr && a.isManualArea);
    if (!data || !data.loaded) {
        if (!hasManualAreas) return null;
        // areaLines のみで画像を生成するため、空の entities で続行
    }
    let entsToDraw = []; let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
    
    console.log(`\n--- [DEBUG] createLayerFilteredImage START (docType: ${docType}, floor: ${floorStr}) ---`);
    console.log(`Target Layers: ${targetLayers.join(', ')}`);

    let targetCount = 0;
    data.entities.forEach(ent => {
        // ★ レイヤ表示判定（印刷時は強制表示）
        if (!isPrint && layerVisibility[ent.layer] === false) return;

        let originL = ent.layer || "";
        let L = originL.normalize("NFKC").toUpperCase().trim();
        
        // ★ 課題3: 階層フィルタバイパス (BG_で始まるレイヤは常に描画対象候補とする)
        let isTarget = targetLayers.some(tl => L.includes(tl.toUpperCase().trim()));
        let isBg = false;
        if (isPrint) {
            isBg = bgLayers.some(bl => L.includes(bl.toUpperCase().trim()));
        } else {
            let isBGPrefix = L.includes('BG_');
            isBg = isBGPrefix || bgLayers.some(bl => L.includes(bl.toUpperCase().trim()));
        }

        if (isTarget) {
            targetCount++;
            console.log(`  - 候補図形: Layer[${originL} -> ${L}], Type[${ent.type}], Closed[${ent.closed}]`);
        }

        if (isTarget || isBg) {
            entsToDraw.push({ ...ent, isBg: isBg });
            if (isTarget) {
                if (ent.type === 'LINE' && ent.vertices) { ent.vertices.forEach(v => { if (v.x != null && !isNaN(v.x)) { if (v.x < minX) minX = v.x; if (v.x > maxX) maxX = v.x; if (v.y < minY) minY = v.y; if (v.y > maxY) maxY = v.y; } }); }
                else if (['LWPOLYLINE', 'POLYLINE'].includes(ent.type) && ent.vertices) { ent.vertices.forEach(v => { if (v.x != null && !isNaN(v.x)) { if (v.x < minX) minX = v.x; if (v.x > maxX) maxX = v.x; if (v.y < minY) minY = v.y; if (v.y > maxY) maxY = v.y; } }); }
                else if (ent.type === 'ARC' || ent.type === 'CIRCLE') { if (ent.center.x - ent.radius < minX) minX = ent.center.x - ent.radius; if (ent.center.x + ent.radius > maxX) maxX = ent.center.x + ent.radius; if (ent.center.y - ent.radius < minY) minY = ent.center.y - ent.radius; if (ent.center.y + ent.radius > maxY) maxY = ent.center.y + ent.radius; }
                else if (['TEXT', 'MTEXT'].includes(ent.type)) { const pos = ent.startPoint || ent.position || ent.insertionPoint || ent.insert || {}; const tx = pos.x ?? 0, ty = pos.y ?? 0; if (tx != null && !isNaN(tx)) { if (tx < minX) minX = tx; if (tx > maxX) maxX = tx; if (ty < minY) minY = ty; if (ty > maxY) maxY = ty; } }
            }
        }
    });
    
    console.log(`[DEBUG] 抽出した描画対象図形数: ${entsToDraw.length} (うちTarget該当数: ${targetCount})`);

    // ★ 修正2: draw-area で作図した areaLines の頂点を BoundingBox に必ず含める
    if (floorStr) {
    ...
    console.log(`[DEBUG] 算出された Bounding Box: minX=${minX}, maxX=${maxX}, minY=${minY}, maxY=${maxY}`);

    if (minX === Infinity || maxX === -Infinity) {
        if (targetLayers.some(tl => tl.includes('AREA'))) {
            console.warn(`[ERROR警告] 床面積の図形が1つも抽出できませんでした (Bounding Box が不正)。`);
        }
        return null;
    }

    // docTypeが 'elev' の時のみ立面スケールを使用。他は平面図スケールを使用。
    ...
    minX -= padCAD; maxX += padCAD; minY -= padCAD; maxY += padCAD;
    let cW = (maxX - minX) * sf, cH = (maxY - minY) * sf;
    ...
    let c = document.createElement('canvas'); c.width = cW; c.height = cH; let x = c.getContext('2d');
    x.fillStyle = '#ffffff'; x.fillRect(0, 0, cW, cH);
    const toC = (xPos, yPos) => ({ cx: (xPos - minX) * sfFinal, cy: cH - ((yPos - minY) * sfFinal) });

    let bgEnts = entsToDraw.filter(e => e.isBg);
    let targetEnts = entsToDraw.filter(e => !e.isBg);
    ...
    if (bgEnts.length > 0) {
        ...
        _drawCADEntities(bgCtx, bgEnts, toC, true, sfFinal, true);
    ...
    // ★ 面積図の追加描画（カラー塗りつぶし ＆ 寸法）
    if (floorStr) {
        const fAreas = areaLines.filter(a => a.floor === floorStr);
        fAreas.forEach((a, index) => {
            x.beginPath();
            a.vertices.forEach((v, i) => { let p = toC(v.x, v.y); if (p.cx != null && !isNaN(p.cx)) { i === 0 ? x.moveTo(p.cx, p.cy) : x.lineTo(p.cx, p.cy); } });
            if (a.closed) x.closePath();
            
            x.save();
            let fillCol = 'rgba(173, 216, 230, 0.4)'; // 水色 (floor)
            ...
            x.save();
            x.font = `bold ${Math.round(20 * dimScale)}px sans-serif`; x.textAlign = "center"; x.textBaseline = "middle";
            ...
            // 重心・寸法描画
            if (showAreaDims) {
                x.font = `bold ${Math.round(18 * dimScale)}px sans-serif`; x.textAlign = "center"; x.textBaseline = "alphabetic";
                for (let i = 0; i < a.vertices.length; i++) {
                    ...
                    if (p1.cx != null && p2.cx != null) {
                        ...
                        x.strokeStyle = '#ffffff'; x.lineWidth = 4 * dimScale;
                        x.strokeText(String(d), mx + nx * offset, my + ny * offset);
                        x.fillStyle = '#c0392b'; x.fillText(String(d), mx + nx * offset, my + ny * offset);
                    }
                }
            }
        });
    ...
    if (targetEnts.length > 0) {
        _drawCADEntities(x, targetEnts, toC, false, sfFinal);
    }

    // ★ 課題2: スケールをキャンバス right下に描画（サブ図面用）
    const scaleInputIdText = (docType === 'elev') ? 'scale-sub' : (document.getElementById('scale-plan') ? 'scale-plan' : 'scale-floor');
    const sScaleVal = document.getElementById(scaleInputIdText) ? document.getElementById(scaleInputIdText).value : '100';
    x.font = `bold ${Math.round(24 * (sfFinal / sf))}px sans-serif`; x.fillStyle = "#333"; x.textAlign = "right";
    x.fillText(`S = 1 / ${sScaleVal}`, c.width - 40, c.height - 40);

    let physW = (maxX - minX) / userScale;
    let physH = (maxY - minY) / userScale;
...
function createHighResPlanImage(f, mode, d4 = null, showAreaDims = true, dimScale = 1.0, isPrint = false) {
    try {
        ...
        if (mode !== 'area') {
            ...
            walls.filter(w => w.floor === f).forEach(w => {
                ...
                if (p1.cx != null && !isNaN(p1.cx) && p2.cx != null && !isNaN(p2.cx)) {
                    ...
                    let tv = w.totalVal || 0, mX = (p1.cx + p2.cx) / 2, mY = (p1.cy + p2.cy) / 2;
                    // ★ 面材記号を描画（数値は非表示）
        ...
        return { img: c.toDataURL('image/png'), physW, physH };
    } catch (e) { console.error("配置図生成エラー:", e); return null; }
}
...
async function generateDoc() {
    try {
        ...
        ['1F', '2F'].forEach(f => {
            ...
            let b = get4DivBounds(f), kxt = 0, kyt = 0, vxt = 0, vxb = 0, vyl = 0, vyr = 0;

            walls.filter(w => w.floor === f).forEach(w => {
                let dx = Math.abs(w.p2.x - w.p1.x), dy = Math.abs(w.p2.y - w.p1.y);
                let L_wall = Math.sqrt(dx * dx + dy * dy) / 1000;
                if (L_wall === 0) return;
                let ratioX = Math.min(1, Math.max(-1, dx / 1000 / L_wall));
                let ratioY = Math.min(1, Math.max(-1, dy / 1000 / L_wall));
                let degX = Math.acos(ratioX) * 180 / Math.PI, degY = Math.acos(ratioY) * 180 / Math.PI;
                let effMx = Math.floor(w.totalVal * (ratioX * ratioX) * 100) / 100;
                let effMy = Math.floor(w.totalVal * (ratioY * ratioY) * 100) / 100;
            ...
            let wallOk = (kxt >= rX) && (kyt >= rY);

            let req_xt = ext * cq, req_xb = exb * cq, req_yl = eyl * cq, req_yr = eyr * cq;
            let rt_xt = vxt / (req_xt || 1), rt_xb = vxb / (req_xb || 1), rt_yl = vyl / (req_yl || 1), rt_yr = vyr / (req_yr || 1);
            let r_x = Math.min(rt_xt, rt_xb) / (Math.max(rt_xt, rt_xb) || 1), r_y = Math.min(rt_yl, rt_yr) / (Math.max(rt_yl, rt_yr) || 1);
            let div4Ok = (r_x >= 0.5 || (rt_xt >= 1.0 && rt_xb >= 1.0)) && (r_y >= 0.5 || (rt_yl >= 1.0 && rt_yr >= 1.0));

            let hasNgLambda = pillars.some(p => !p.isDeleted && !p.isInvalidPos && p.floor === f && p.lambda != null && !p.lambdaOK);
            let lamOk = !hasNgLambda;

            let floorOk = wallOk && div4Ok && lamOk;
        ...
        ['1F', '2F'].forEach(f => {
            ...
            walls.filter(w => w.floor === f).forEach(w => {
                ...
                let tvStr = '';
                let braceLabel = getBraceLabel(w.braceName, bV, w);
        ...
        ['1F', '2F'].forEach(f => {
            let xt = [], xb = [], ylf = [], yrg = [], txt = 0, txb = 0, tyl = 0, tyr = 0, b = get4DivBounds(f);
            walls.filter(w => w.floor === f).forEach(w => {
                let dx = Math.abs(w.p2.x - w.p1.x), dy = Math.abs(w.p2.y - w.p1.y);
                ...
                let effMx = Math.floor(w.totalVal * (rx * rx) * 100) / 100;
                let effMy = Math.floor(w.totalVal * (ry * ry) * 100) / 100;
                let effX = effMx, effY = effMy;
                ...
                if (effX > 0 && b && cy >= b.yTop) { xt.push(`<tr><td>${w.p1.gy} ${w.p1.gx}-${w.p2.gx}${isDiag}</td><td>${L.toFixed(3)}</td><td>${kx.toFixed(2)}</td></tr>`); txt += kx; }
                if (effX > 0 && b && cy <= b.yBottom) { xb.push(`<tr><td>${w.p1.gy} ${w.p1.gx}-${w.p2.gx}${isDiag}</td><td>${L.toFixed(3)}</td><td>${kx.toFixed(2)}</td></tr>`); txb += kx; }
                if (effY > 0 && b && cx <= b.xLeft) { ylf.push(`<tr><td>${w.p1.gx} ${w.p1.gy}-${w.p2.gy}${isDiag}</td><td>${L.toFixed(3)}</td><td>${ky.toFixed(2)}</td></tr>`); tyl += ky; }
                if (effY > 0 && b && cx >= b.xRight) { yrg.push(`<tr><td>${w.p1.gx} ${w.p1.gy}-${w.p2.gy}${isDiag}</td><td>${L.toFixed(3)}</td><td>${ky.toFixed(2)}</td></tr>`); tyr += ky; }
            });
