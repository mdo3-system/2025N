...
window.FoundationInputController = {
    ...
    handleBeamInput: function(snap, state) {
        if (!state.fdSelectedPillarLike) {
            ...
            // [超重要] 基礎梁は最初の交点と最後の交点を結ぶ直線とする。
            // 以前の直交化クランプ（dx > dy 等）は斜め梁を許容するために削除。
            /* 
    ...
    trySelectElement: function(mx, my, state) {
        ...
        for (const b of (state.foundationBeams || [])) {
            // [改善] 基礎梁の選択をスパン（投影後の座標）ベースで行うように強化
            if (b.spans && b.spans.length > 0) {
                for (const s of b.spans) {
                    if (!s.startNode || !s.endNode) continue;
                    const l2 = (s.endNode.x - s.startNode.x) ** 2 + (s.endNode.y - s.startNode.y) ** 2;
                    const t = l2 > 0 ? Math.max(0, Math.min(1, ((wx - s.startNode.x) * (s.endNode.x - s.startNode.x) + (wy - s.startNode.y) * (s.endNode.y - s.startNode.y)) / l2)) : 0;
                    if (Math.hypot(wx - (s.startNode.x + t * (s.endNode.x - s.startNode.x)), wy - (s.startNode.y + t * (s.endNode.y - s.startNode.y))) < HIT * 1.5) { 
                ...
                if (Math.hypot(wx - (b.p1.x + t * (b.p2.x - b.p1.x)), wy - (b.p1.y + t * (b.p2.y - b.p1.y))) < HIT * 1.5) { 
                    window.PropertyController.showFdPopup('beam', b, mx, my); 
                    return true; 
                }
            }
